...
function piErrorMessage(data, fallback) {
...
async function approvePayment(request, env) {
  ...
  if (!body.paymentId || !body.paymentIntentId) return json({ error: 'paymentId and paymentIntentId are required' }, 400, request);
  ...
  if (!intent || new Date(intent.expires_at) <= new Date()) return json({ error: 'Payment intent is invalid or expired' }, 409, request);
  if (intent.pi_payment_id && intent.pi_payment_id !== body.paymentId) return json({ error: 'Payment ID does not match intent' }, 409, request);
  const response = await piFetch(env, `/payments/${encodeURIComponent(body.paymentId)}`);
  const payment = await response.json().catch(() => ({}));
  if (!response.ok) return json({ error: piErrorMessage(payment, 'Unable to verify Pi payment'), piStatus: response.status }, 502, request);
  const status = validatePayment(payment, { ...intent, pi_payment_id: body.paymentId }, user);
  if (status.cancelled || status.user_cancelled || status.developer_completed) return json({ error: 'Pi payment is not approvable in its current state' }, 409, request);
  if (!status.developer_approved) {
    ...
    if (!approvedResponse.ok) return json({ error: piErrorMessage(approved, 'Pi payment approval failed'), piStatus: approvedResponse.status }, 502, request);
  }
  ...
  return json({ approved: true, paymentId: body.paymentId }, 200, request);
}
async function completePayment(request, env) {
  const user = await requireUser(request, env);
  const body = await readJson(request);
  if (!body.paymentId || !body.txid || !body.paymentIntentId) return json({ error: 'paymentId, txid and paymentIntentId are required' }, 400, request);
  const intent = await env.RENTORA_DB.prepare('SELECT * FROM payment_intents WHERE id=?1 AND user_id=?2 LIMIT 1').bind(body.paymentIntentId, user.id).first();
  if (!intent) return json({ error: 'Payment intent not found' }, 404, request);
  if (intent.pi_payment_id && intent.pi_payment_id !== body.paymentId) return json({ error: 'Payment ID does not match intent' }, 409, request);
  const response = await piFetch(env, `/payments/${encodeURIComponent(body.paymentId)}`);
  const payment = await response.json().catch(() => ({}));
  if (!response.ok) return json({ error: piErrorMessage(payment, 'Unable to verify Pi payment before completion'), piStatus: response.status }, 502, request);
  const status = validatePayment(payment, { ...intent, pi_payment_id: body.paymentId }, user);
  if (status.cancelled || status.user_cancelled) return json({ error: 'Pi payment is cancelled' }, 409, request);
  ...
  if (!status.developer_completed) {
    ...
    if (!completionResponse.ok || !completion?.status?.developer_completed) return json({ error: piErrorMessage(completion, 'Pi payment completion failed'), piStatus: completionResponse.status }, 502, request);
  ...
  return json({ completed: true, paymentId: body.paymentId, txid: body.txid }, 200, request);
}
async function adminRoute(request, env, path) {
  const user = await requireUser(request, env);
  if (!(adminAllowed(user.pi_uid, env) || adminAllowed(user.username, env))) return json({ error: 'Admin access required' }, 403, request);
  if (path === '/api/auth/me') return json({ authenticated: true, user: { ...userView(user, env), isAdmin: true }, isAdmin: true }, 200, request);
  if (path === '/api/admin/users') {
    const rows = await env.RENTORA_DB.prepare('SELECT * FROM users ORDER BY created_at DESC').all();
    return json({ success: true, users: (rows.results || []).map((row) => userView(row, env)) }, 200, request);
  }
  if (path === '/api/admin/overview') {
    ...
    return json({ success: true, overview: { totalUsers: Number(usersCount?.c || 0), totalListings: Number(listingsCount?.c || 0), totalRentals: Number(rentalsCount?.c || 0), totalTransactions: Number(transactionsCount?.c || 0), totalPlatformRevenue: Number(revRow?.total || 0), openReports: 0 } }, 200, request);
  }
...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      return json({ error: err?.message || 'Server error' }, Number(err?.status) || 500, request);
    }
  }
};
