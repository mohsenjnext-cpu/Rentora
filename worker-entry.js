...
function validatePayment(payment, intent, user) {
  ...
  const payerUid = payment?.user_uid || payment?.user?.uid || payment?.from_address?.uid;
  if (String(payerUid || '') !== String(user.pi_uid)) {
...
function calculateServerQuote(listing, startDate, endDate, env) {
  ...
  const dailyRate = Number(listing.price_per_day);
  const deposit = Number(listing.deposit_amount || 0);
  if (!Number.isFinite(dailyRate) || dailyRate < 0 || !Number.isFinite(deposit) || deposit < 0) {
    throw Object.assign(new Error('Listing has invalid pricing'), { status: 409 });
  }
  ...
  let feeRate = Number(listing.platform_fee_rate);
  if (!Number.isFinite(feeRate) || feeRate <= 0) feeRate = Number(env.PLATFORM_FEE_RATE || 0.05);
  feeRate = Math.max(0, Math.min(0.5, feeRate));
  const feeUnits = rentalUnits > 0 ? Math.max(1, Math.round(rentalUnits * feeRate)) : 1;

  return {
    days,
    dailyRate: dailyUnits / MONEY_SCALE,
    rentalTotal: rentalUnits / MONEY_SCALE,
    deposit: depositUnits / MONEY_SCALE,
    platformFeeRate: feeRate,
    platformFee: feeUnits / MONEY_SCALE,
    totalAmount: feeUnits / MONEY_SCALE,
  };
}
...
function rentalView(row) {
  ...
  return {
    ...meta,
    id: row.id,
    bookingNumber: meta.bookingNumber || `RN-${String(row.id).slice(-8)}`,
    agreementId: meta.agreementId || null,
    itemId: row.listing_id,
    itemTitle: row.title,
    itemCategory: row.category,
    itemImage: meta.itemImage || (Array.isArray(meta.images) ? meta.images[0] : meta.images) || '',
    ...
    daysCount: meta.daysCount || Math.max(1, Math.ceil((Date.parse(`${row.end_date}T00:00:00Z`) - Date.parse(`${row.start_date}T00:00:00Z`)) / 86400000)),
    ...
    paymentDueToRentora: row.platform_fee,
    status: row.status,
    paymentStatus: row.payment_status,
    piPaymentId: row.pi_payment_id || meta.piPaymentId || null,
    piTxRef: row.pi_txid || meta.piTxRef || null,
...
async function loadRental(env, rentalId) {
  return env.RENTORA_DB.prepare(`
    SELECT r.*, l.title, l.category, l.location, l.price_per_day, l.deposit_amount, l.platform_fee_rate,
           ru.pi_uid AS renter_pi_uid, ru.username AS renter_username,
           ou.pi_uid AS owner_pi_uid, ou.username AS owner_username, ou.avatar_url AS owner_avatar
    FROM rentals r
...
async function createRental(request, env, origin) {
  ...
  if (!listingId) return error('listingId is required', 400, env, {}, origin);
  if (!startDate || !endDate) return error('Valid startDate and endDate are required', 400, env, {}, origin);

  const listing = await env.RENTORA_DB.prepare(`
    SELECT l.*, u.pi_uid AS owner_pi_uid, u.username AS owner_username
    FROM listings l JOIN users u ON u.id=l.owner_user_id
  ...
  if (listing.owner_user_id === user.id || String(listing.owner_pi_uid) === String(user.pi_uid)) {
    return error('Self-booking is not allowed', 400, env, {}, origin);
  }

  const quote = calculateServerQuote(listing, startDate, endDate, env);
  const id = `rental_${crypto.randomUUID()}`;
  const bookingNumber = `RN-${crypto.randomUUID().replace(/-/g, '').slice(0, 8).toUpperCase()}`;
  const agreementId = `AGR-${crypto.randomUUID().replace(/-/g, '').slice(0, 10).toUpperCase()}`;
  const createdAt = now();
  ...
  const metadata = {
    ...
    bookingNumber,
    agreementId,
    daysCount: quote.days,
    itemImage: typeof body?.itemImage === 'string' ? body.itemImage : '',
    deliveryRequired: Boolean(body?.deliveryRequired),
  ...
  try {
    await env.RENTORA_DB.prepare(`
      INSERT INTO rentals(
        id, listing_id, renter_user_id, start_date, end_date,
        rental_amount, deposit_amount, platform_fee, total_amount,
        status, payment_status, metadata, created_at, updated_at
      ) VALUES(?1,?2,?3,?4,?5,?6,?7,?8,?8,'pending_payment','unpaid',?9,?10,?10)
    `).bind(
      id, listingId, user.id, startDate, endDate,
      quote.rentalTotal, quote.deposit, quote.platformFee,
      JSON.stringify(metadata), createdAt, createdAt
    ).run();
  } catch (err) {
    if (/already reserved|overlap|UNIQUE|constraint/i.test(String(err?.message || err))) {
      return error('Listing is already reserved for the requested dates', 409, env, {}, origin);
    }
    throw err;
  }

  const rental = await loadRental(env, id);
  return json({ success: true, rental: rentalView(rental) }, 201, env, origin);
}
...
async function updateRentalStatus(request, env, origin) {
  ...
  const rental = await loadRental(env, rentalId);
  if (!rental) return error('Rental not found', 404, env, {}, origin);
  const isOwner = rental.owner_user_id === user.id;
  const isRenter = rental.renter_user_id === user.id;
  if (!isOwner && !isRenter) return error('Access denied to rental', 403, env, {}, origin);

  let nextStatus;
  if (action === 'handover') {
    if (!['confirmed', 'active'].includes(rental.status)) return error('Rental must be confirmed before handover', 409, env, {}, origin);
    if (rental.payment_status !== 'completed') return error('Payment must be confirmed before handover', 409, env, {}, origin);
    if (rental.status === 'active') return json({ success: true, idempotent: true, rental: rentalView(rental) }, 200, env, origin);
    nextStatus = 'active';
  } else {
    if (!isOwner) return error('Only the listing owner can confirm return', 403, env, {}, origin);
    if (rental.status === 'completed') return json({ success: true, idempotent: true, rental: rentalView(rental) }, 200, env, origin);
    if (rental.status !== 'active') return error('Rental must be active before return', 409, env, {}, origin);
    nextStatus = 'completed';
  }

  const metadata = parseMetadata(rental.metadata);
  if (action === 'handover') metadata.handoverConfirmedAt = now();
  if (action === 'return') metadata.returnConfirmedAt = now();
  ...
  const updated = await loadRental(env, rentalId);
  return json({ success: true, idempotent: false, rental: rentalView(updated) }, 200, env, origin);
}
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (!isOriginAllowed(origin, request.url, env)) return error('Origin not allowed', 403, env, {}, origin);
    if (request.method === 'POST' && url.pathname === '/api/sync/rental') {
      try { return await createRental(request, env, origin); } catch (err) { return error(err?.message || 'Server error', Number(err?.status) || 500, env, {}, origin); }
    }
    if (request.method === 'POST' && url.pathname === '/api/sync/rental/status') {
      try { return await updateRentalStatus(request, env, origin); } catch (err) { return error(err?.message || 'Server error', Number(err?.status) || 500, env, {}, origin); }
    }
