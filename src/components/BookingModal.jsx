          ...
          {confirmedBookingData ? (
            <div className="space-y-4 animate-fadeIn">
              ...
              <div className="p-4 rounded-xl rentora-card space-y-3 border border-[#0F6E56]/30 bg-gradient-to-b from-white to-emerald-500/5 dark:from-[#121124] dark:to-emerald-950/10">
                ...
                {confirmedBookingData.contact?.contactPhone && <div className="flex items-center justify-between text-xs pt-1 border-t border-slate-100 dark:border-slate-800"><div><span className="text-[10px] text-slate-400 block">{l('شماره تماس مستقیم:', 'Phone:', 'الهاتف:', '电话：')}</span><strong className="font-mono font-bold text-slate-900 dark:text-white" dir="ltr">{confirmedBookingData.contact.contactPhone}</strong></div><div className="flex items-center gap-1.5"><button type="button" onClick={() => handleCopyPhone(confirmedBookingData.contact.contactPhone)} className="min-h-11 min-w-11 px-2 py-1 rounded bg-[#EEEDFE] dark:bg-[#26215C] text-[#26215C] dark:text-[#EEEDFE] text-[10px] font-bold cursor-pointer">{copiedPhone ? l('کپی شد', 'Copied', 'تم النسخ', '已复制') : l('کپی', 'Copy', 'نسخ', '复制')}</button><a href={`tel:${confirmedBookingData.contact.contactPhone}`} className="btn-primary min-h-11 px-2.5 py-1 text-[11px] font-bold flex items-center gap-1"><Phone className="w-3 h-3" /><span>{l('تماس', 'Call', 'اتصال', '拨打')}</span></a></div></div>}
            ...
            <form onSubmit={handleCreateBooking} className="space-y-4">
              <div className="p-3 rounded-xl rentora-card flex items-center gap-3">{item.images?.[0] ? <img src={item.images[0]} alt="" className="w-12 h-12 rounded-lg object-cover border border-slate-200 dark:border-slate-700 shrink-0" /> : <div aria-hidden="true" className="w-12 h-12 rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-100 dark:bg-slate-800 shrink-0 flex items-center justify-center"><Receipt className="w-5 h-5 text-slate-400" /></div>}<div className="min-w-0 flex-1"><h4 className="font-bold text-xs text-slate-900 dark:text-white truncate">{item.title}</h4><div className="text-[11px] text-slate-400 flex items-center gap-1 mt-0.5"><span className="font-mono text-[#0F6E56] dark:text-[#48D2A8] font-bold">{dailyPrice ?? '—'} π / {l('روز', 'day', 'يوم', '天')}</span><span>•</span><span className="truncate">{item.location || '—'}</span></div></div></div>
