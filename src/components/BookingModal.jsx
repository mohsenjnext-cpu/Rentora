      ...
      >
        <div className="p-4 sm:p-5 border-b border-slate-150 dark:border-slate-800 flex items-center justify-between bg-slate-50/70 dark:bg-[#16152B]/70">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="font-bold text-sm sm:text-base text-slate-900 dark:text-white">{t('bookingModalTitle')}</h3>
              <p className="text-[11px] text-slate-400">{l('محاسبه شفاف و پرداخت آنلاین کارمزد در شبکه پای', 'Transparent quote with Pi fee payment', 'حساب شفاف ودفع عمولة رنتورا عبر باي', '明晰透明报价，在线支付 Pi 平台费')}</p>
            </div>
          </div>
          <button type="button" onClick={onClose} className="p-1.5 rounded-lg text-slate-400 hover:text-slate-700 dark:hover:text-white transition cursor-pointer"><X className="w-5 h-5" /></button>
        </div>
            ...
            <div className="space-y-4 animate-fadeIn">
              ...
              <div className="p-4 rounded-xl rentora-card space-y-3 border-2 border-[#534AB7]/30 bg-gradient-to-b from-white to-[#EEEDFE]/20 dark:from-[#121124] dark:to-[#1C1B33]/40">
                ...
                <div className="space-y-1.5 pt-2 border-t border-slate-150 dark:border-slate-800 text-xs">
                  <div className="flex justify-between"><span className="text-slate-500">{l('کالا و مدت:', 'Item & Period:', 'الغرض والمدة:', '物品及租期：')}</span><span className="font-semibold text-slate-900 dark:text-white">{item.title} ({pricing.daysCount} {l('روز', 'days', 'أيام', '天')})</span></div>
              ...
              <button type="button" onClick={onClose} className="btn-primary w-full py-2.5 text-xs font-bold cursor-pointer">{l('مشاهده در بخش فعالیت‌ها', 'View in Activity', 'عرض في قسم النشاطات', '在活动列表中查看')}</button>
            </div>
          ) : (
