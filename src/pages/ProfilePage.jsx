...
export default function ProfilePage({ onNavigate, onSelectItem, onOpenPublicProfile }) {
  ...
  useEffect(() => {
    ...
    let mounted = true;
    fetchUserReviews(currentUser.username).then(data => mounted && data && setUserReviewsData(data)).catch(() => {});
    return () => { mounted = false; };
  }, [currentUser?.username, fetchUserReviews]);
    ...
    <div className="w-full max-w-6xl mx-auto space-y-5 pb-16 animate-fadeIn">
      ...
      <div className="flex items-center justify-between gap-3">
        ...
        <div className="flex gap-2">
          ...
          <button type="button" onClick={() => onNavigate('settings')} className="p-2 rounded-xl border border-slate-200 dark:border-slate-700"><Settings className="w-4 h-4" /></button>
        </div>
      </div>
      ...
      <section className="grid lg:grid-cols-[minmax(0,1fr)_300px] gap-5">
        <div className="rentora-card rounded-3xl p-5 sm:p-7">
          {isEditing ? <form onSubmit={save} className="space-y-5">
            ...
            <div className="relative shrink-0"><Avatar src={currentUser.avatar} username={currentUser.username} className="w-24 h-24" /><button type="button" onClick={() => fileInputRef.current?.click()} className="absolute -bottom-1 -right-1 w-8 h-8 rounded-xl bg-[#26215C] text-white border-2 border-white dark:border-[#151426] flex items-center justify-center"><Camera className="w-4 h-4" /></button></div>
