...
test('Phase 4: Pi Testnet Environment Configuration', async (t) => {
  await t.test('Pi SDK initialization in piService is strictly configured for Pi Testnet (sandbox: false)', () => {
    ...
    // Verify sandbox: false is explicitly configured
    assert.match(piServiceContent, /sandbox:\s*false/, 'piService must have sandbox: false');

    // Verify sandbox: true is nowhere present
...
test('Phase 4: onIncompletePaymentFound Handling and Server Recovery', async (t) => {
  ...
  await t.test('Server gateway handles incomplete payment resolution via Pi API', () => {
    const gatewayPath = path.join(rootDir, 'worker-gateway2.js');
    const workerPath = path.join(rootDir, '_worker.js');
...
test('Phase 4: Domain & App Studio Validation Key', async (t) => {
  ...
  await t.test('All backend entrypoints serve validation-key.txt identically', () => {
    ...
    assert.ok(serverContent.includes(expectedKey), 'server.js must serve canonical validation key');
  });
});
