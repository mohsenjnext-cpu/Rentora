...
test('Admin A2U queues reconciliation when Pi completes without verified txid', async () => {
  const db = createMockDb(), kv = createMockKv(), token = await setupSession(kv, db.users[0]), env = envFor(db, kv);
  const originalFetch = globalThis.fetch;
  globalThis.fetch = async (url, opts) => {
    const u = String(url);
    if (u.includes('/payments/incomplete_server_payments')) return new Response(JSON.stringify({ incomplete_server_payments: [] }), { status: 200 });
    if (u.endsWith('/payments') && opts?.method === 'POST') return new Response(JSON.stringify({ identifier: 'reconcile_queue_1', amount: 5, status: { developer_approved: true, developer_completed: true, transaction_verified: false }, transaction: null }), { status: 200 });
    if (u.endsWith('/payments/reconcile_queue_1')) return new Response(JSON.stringify({ identifier: 'reconcile_queue_1', amount: 5, status: { developer_approved: true, developer_completed: true, transaction_verified: false }, transaction: null }), { status: 200 });
    return originalFetch(url, opts);
  };
  try {
    const res = await worker.fetch(new Request('http://localhost/api/admin/payout', {
      method: 'POST',
      headers: { Authorization: 'Bearer ' + token, 'Content-Type': 'application/json', 'Idempotency-Key': 'reconcile_queue_1' },
      body: JSON.stringify({ amount: 5 })
    }), env);
    assert.equal(res.status, 502);
    assert.equal(db.transactions.filter(t => t.type === 'admin_payout').length, 0);
    assert.equal(db.payoutReconciliationQueue?.length || 0, 1);
  } finally {
    globalThis.fetch = originalFetch;
  }
});
