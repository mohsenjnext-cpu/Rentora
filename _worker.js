...
function userView(row) { const meta = parseMetadata(row.metadata); return { ...meta, id: row.id, uid: row.pi_uid, piUid: row.pi_uid, username: row.username, displayName: row.display_name, avatar: row.avatar_url, role: row.role, status: row.status, kycStatus: meta.kycStatus === 'verified' ? 'verified' : 'unverified', isOfficialSdk: true, joinedDate: row.created_at ? row.created_at.slice(0, 10) : '' }; }
...
export default {
  async fetch(request, env, ctx) {
    ...
    } catch (err) { console.error('Rentora worker error', err); const status = Number(err?.status) || (String(err?.message || '').includes('body too large') ? 413 : 500); return errorResponse(status === 500 ? 'Server error' : err.message, status, env, undefined, origin); }
  }
};
