...
function userView(row, env) {
  ...
  return {
    ...
    kycStatus: meta.kycStatus === 'verified' || row.kyc_status === 'verified' ? 'verified' : 'unverified',
    isOfficialSdk: true,
...
async function listAll(env, auth) {
  ...
  const isAdm = user ? (isAdmin(user.pi_uid, env) || isAdmin(user.username, env)) : false;

  let itemsQuery;
  if (isAdm) {
    itemsQuery = env.RENTORA_DB.prepare(`SELECT l.*, u.pi_uid owner_pi_uid, u.username owner_username, u.avatar_url owner_avatar FROM listings l JOIN users u ON u.id=l.owner_user_id WHERE l.status != 'deleted' ORDER BY l.created_at DESC`);
  } else if (user) {
    itemsQuery = env.RENTORA_DB.prepare(`SELECT l.*, u.pi_uid owner_pi_uid, u.username owner_username, u.avatar_url owner_avatar FROM listings l JOIN users u ON u.id=l.owner_user_id WHERE (l.status = 'active') OR (l.owner_user_id = ?1 AND l.status != 'deleted') ORDER BY l.created_at DESC`).bind(user.id);
  } else {
    itemsQuery = env.RENTORA_DB.prepare(`SELECT l.*, u.pi_uid owner_pi_uid, u.username owner_username, u.avatar_url owner_avatar FROM listings l JOIN users u ON u.id=l.owner_user_id WHERE l.status = 'active' ORDER BY l.created_at DESC`);
  }

  let rentalsPromise, transactionsPromise, usersPromise;
  if (isAdm) {
    rentalsPromise = env.RENTORA_DB.prepare(`SELECT r.*, l.price_per_day, ru.pi_uid renter_pi_uid, ru.username renter_username, ou.pi_uid owner_pi_uid, ou.username owner_username FROM rentals r JOIN listings l ON l.id=r.listing_id JOIN users ru ON ru.id=r.renter_user_id JOIN users ou ON ou.id=l.owner_user_id ORDER BY r.created_at DESC`).all();
    transactionsPromise = env.RENTORA_DB.prepare(`SELECT t.*, u.pi_uid user_pi_uid FROM transactions t JOIN users u ON u.id=t.user_id ORDER BY t.created_at DESC`).all();
    usersPromise = env.RENTORA_DB.prepare('SELECT * FROM users ORDER BY created_at DESC').all();
  } else if (user) {
    ...
    transactionsPromise = env.RENTORA_DB.prepare(`SELECT t.*, u.pi_uid user_pi_uid FROM transactions t JOIN users u ON u.id=t.user_id WHERE t.user_id=?1 ORDER BY t.created_at DESC`).bind(user.id).all();
    usersPromise = Promise.resolve({ results: [user] });
  } else {
    rentalsPromise = Promise.resolve({ results: [] });
    transactionsPromise = Promise.resolve({ results: [] });
    usersPromise = Promise.resolve({ results: [] });
  }
  ...
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      if (method === 'POST' && path === '/api/auth/pi-login') {
        ...
        return jsonResponse({ authenticated: true, verifiedWithPiApi: true, user: userView(user), sessionToken, uid, username }, 200, env);
      }
      if (method === 'POST' && path === '/api/auth/logout') { requireBindings(env); const header = request.headers.get('Authorization') || ''; if (header.startsWith('Bearer ')) { const token = header.slice(7).trim(); if (token) await env.RENTORA_KV.delete(`session:${await sha256(token)}`); } return jsonResponse({ success: true }, 200, env); }
      if (method === 'POST' && path === '/api/payments/intent') { const { user } = await requireUser(request, env); const body = await readJson(request); if (!body.rentalId) return errorResponse('rentalId is required', 400, env); const rental = await env.RENTORA_DB.prepare(`SELECT r.*, l.title, l.id listing_id, l.price_per_day, l.deposit_amount FROM rentals r JOIN listings l ON l.id=r.listing_id WHERE r.id=?1 AND r.renter_user_id=?2 LIMIT 1`).bind(body.rentalId, user.id).first(); if (!rental) return errorResponse('Rental not found', 404, env); if (!['draft','pending_payment'].includes(rental.status)) return errorResponse('Rental is not payable', 409, env); const existing = await env.RENTORA_DB.prepare('SELECT * FROM payment_intents WHERE rental_id=?1 LIMIT 1').bind(rental.id).first(); if (existing && existing.status !== 'cancelled' && new Date(existing.expires_at) > new Date()) return jsonResponse(existing, 200, env); const id = `pii_${crypto.randomUUID()}`; const memo = `Rentora Fee #${String(rental.id).slice(-12)}`; const expires = new Date(Date.now() + PAYMENT_INTENT_TTL * 1000).toISOString(); await env.RENTORA_DB.prepare(`INSERT INTO payment_intents(id,rental_id,user_id,amount,memo,status,created_at,expires_at,updated_at) VALUES(?1,?2,?3,?4,?5,'created',?6,?7,?6)`).bind(id, rental.id, user.id, rental.platform_fee, memo, now(), expires).run(); await env.RENTORA_DB.prepare(`UPDATE rentals SET payment_status='pending', status='pending_payment', updated_at=?1 WHERE id=?2`).bind(now(), rental.id).run(); await env.RENTORA_KV.put(`payment-intent:${id}`, JSON.stringify({ userId: user.id, rentalId: rental.id, amount: rental.platform_fee, memo }), { expirationTtl: PAYMENT_INTENT_TTL }); return jsonResponse({ paymentIntentId: id, amount: rental.platform_fee, memo, expiresAt: expires }, 201, env); }
      if (method === 'POST' && path === '/api/payments/approve') {
        const { user } = await requireUser(request, env); const body = await readJson(request); if (!body.paymentId || !body.paymentIntentId) return errorResponse('paymentId and paymentIntentId are required', 400, env);
        let intent = await env.RENTORA_DB.prepare('SELECT * FROM payment_intents WHERE id=?1 AND user_id=?2 LIMIT 1').bind(body.paymentIntentId, user.id).first(); if (!intent || new Date(intent.expires_at) <= new Date()) return errorResponse('Payment intent is invalid or expired', 409, env); if (intent.status === 'completed') return jsonResponse({ approved: true, paymentId: body.paymentId, idempotent: true }, 200, env);
        ...
        if (intent.status === 'approved' && intent.pi_payment_id === body.paymentId) return jsonResponse({ approved: true, paymentId: body.paymentId, idempotent: true }, 200, env);
        ...
        if (!['created','pending','approved'].includes(status)) return errorResponse(`Pi payment cannot be approved from status ${status || 'unknown'}`, 409, env);
        const approveResponse = await piFetch(env, `/payments/${encodeURIComponent(body.paymentId)}/approve`, { method: 'POST', body: '{}' }); const approved = await approveResponse.json().catch(() => ({})); if (!approveResponse.ok) return errorResponse('Pi payment approval failed', 502, env, { details: approved });
        ...
        if (!Number(claim?.meta?.changes || 0)) { intent = await env.RENTORA_DB.prepare('SELECT * FROM payment_intents WHERE id=?1 AND user_id=?2 LIMIT 1').bind(intent.id, user.id).first(); if (!intent || intent.pi_payment_id !== body.paymentId || !['approved','completed'].includes(intent.status)) return errorResponse('Payment intent was concurrently claimed by another payment', 409, env); }
        return jsonResponse({ approved: true, paymentId: body.paymentId, data: approved, idempotent: Number(claim?.meta?.changes || 0) === 0 }, 200, env);
      }
      ...
      if (method === 'POST' && path === '/api/sync/item') { const { user } = await requireUser(request, env); const item = await readJson(request);
        ...
        if (existing) {
          ...
          return jsonResponse({ success: true, item: { ...sanitizedItem, pricePerDay: price, deposit } }, 200, env);
        }
        ...
        return jsonResponse({ success: true, item: sanitizedItem }, 201, env);
      }
      if (method === 'GET' && path.startsWith('/api/rentals/') && path.endsWith('/contact')) {
        ...
        if (!row) return errorResponse('Rental not found', 404, env);
        ...
        if (!isRenter && !isOwner && !isAdminUser) {
          return errorResponse('Access denied to rental contact details', 403, env);
        }
        ...
        if (isRenter && !isOwner && !isAdminUser) {
          ...
          if (!isPaid || !isValidRentalState) {
            return errorResponse('Contact information is locked until rental payment is confirmed', 403, env);
          }
        }
      ...
      if (method === 'GET' && path.startsWith('/api/listings/') && path.endsWith('/contact')) {
        ...
        if (!row) return errorResponse('Listing not found', 404, env);

        const isOwner = (row.owner_user_id === user.id);
        const isAdminUser = isAdmin(user.pi_uid, env) && user.role === 'admin';
        if (!isOwner && !isAdminUser) {
          return errorResponse('Access denied to listing contact details', 403, env);
        }
      ...
      if (method === 'POST' && path === '/api/conversations') {
        ...
        if (rentalId) {
          ...
          const isAdminUser = isAdmin(user.pi_uid, env) && user.role === 'admin';
          if (!isRenter && !isOwner && !isAdminUser) return errorResponse('Access denied to rental conversation', 403, env);
          ...
          if (!targetListingId) return errorResponse('listingId is required', 400, env);
          ...
          if (listing.owner_user_id === user.id) return errorResponse('Self-conversation is not allowed', 400, env);
        ...
        if (existing) {
          ...
          return jsonResponse({ success: true, conversationId: existing.id, conversation: existing }, 200, env);
        }
        ...
        return jsonResponse({ success: true, conversationId: convId, conversation: created }, 201, env);
      }
      ...
      if (method === 'GET' && path.startsWith('/api/conversations/') && path.endsWith('/messages')) {
        ...
        if (!convId) return errorResponse('Missing conversation ID', 400, env);
        const { user } = await requireUser(request, env);
        ...
        if (!conv) return errorResponse('Conversation not found', 404, env);
        const isParticipant = (conv.owner_user_id === user.id || conv.renter_user_id === user.id);
        const isAdminUser = isAdmin(user.pi_uid, env) && user.role === 'admin';
        if (!isParticipant && !isAdminUser) return errorResponse('Access denied to conversation', 403, env);
      ...
      if (method === 'POST' && path.startsWith('/api/conversations/') && path.endsWith('/messages')) {
        ...
        if (!rawText) return errorResponse('Message text cannot be empty', 400, env);
        if (rawText.length > 2000) return errorResponse('Message is too long (max 2000 characters)', 400, env);
        ...
        const isParticipant = (conv.owner_user_id === user.id || conv.renter_user_id === user.id);
        const isAdminUser = isAdmin(user.pi_uid, env) && user.role === 'admin';
        if (!isParticipant && !isAdminUser) return errorResponse('Access denied to send message in this conversation', 403, env);
      ...
      if (method === 'POST' && path === '/api/sync/user') {
        ...
        return jsonResponse({ success: true, user: userView(updated, env) }, 200, env);
      }
      if (method === 'POST' && path === '/api/upload') {
        ...
        if (!detectedMime) {
          return errorResponse('Invalid image magic bytes. Supported formats: JPEG, PNG, WebP.', 400, env);
        }
